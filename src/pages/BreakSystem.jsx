...
const SHIFT_HOURS_OPTIONS = Array.from({ length: 24 }, (_, i) => ({
  label: `${i.toString().padStart(2, '0')}:00 WITA${i === 0 ? ' (Midnight)' : i === 22 ? ' (Shift Malam - 8 Jam)' : ''}`,
  hour: i
}));
...
export default function BreakSystem() {
  ...
  const [, setActiveShiftStatusIn] = useState('');
  ...
  const [stationCrewModal, setStationCrewModal] = useState({
    isOpen: false,
    stationName: '',
    crewList: []
  });
  const [activeShiftMembers, setActiveShiftMembers] = useState([]);
  ...
  const defaultDates = useMemo(() => getDefaultCutoffRange(), []);
  ...
  // Modal Input Pelanggaran / Ketidakhadiran
  ...
  // State Rekap Presensi & Custom Range Filter
  ...
  const [cutoffAttendanceList, setCutoffAttendanceList] = useState([]);
  const [isFetchingSummary, setIsFetchingSummary] = useState(false);
  const [isFetchingCutoff, setIsFetchingCutoff] = useState(false);
  const [selectedCutoffDetail, setSelectedCutoffDetail] = useState(null);
  const [showCutoffDetailModal, setShowCutoffDetailModal] = useState(false);
  ...
  const fetchOutletsList = useCallback(async () => {
    try {
      ...
      if (!error && data) {
        const filtered = data.filter(o => !/cabang\s*0[2-9]|cabang\s*10/i.test(o.name));
        setOutletsList(filtered);
      }
    } catch (e) {
      console.error("Gagal load outlets:", e);
    }
  }, []);
  ...
  const getCrewBadge = (points) => {
    if (isAreaManager) {
      return { name: 'Executive Manager', color: 'bg-indigo-900 text-indigo-100 border-indigo-700' };
    }
  ...
  const fetchActiveShiftStats = useCallback(async () => {
    try {
      ...
      let logsQuery = supabase
        ...
        .select('id, user_id, status_in, break_start_time, break_end_time, discipline_status, outlet_id, actual_in')
        .not('actual_in', 'is', null)
        .is('actual_out', null)
        .gte('actual_in', past24Hours);
      ...
      if (!lError && !pError && activeLogs && profiles) {
        ...
        const activeMembersArray = [];
        const nowMs = Date.now();

        for (const log of activeLogs) {
          const inTimeMs = new Date(log.actual_in).getTime();
          const inHour = new Date(log.actual_in).getHours();
          const targetHours = (inHour === 22) ? 8 : 9;
          const maxAllowedSessionMs = (targetHours + 3) * 60 * 60 * 1000;
          const elapsedMs = nowMs - inTimeMs;

          if (elapsedMs > maxAllowedSessionMs) {
            const outTimestamp = new Date(inTimeMs + targetHours * 3600 * 1000).toISOString();

            supabase
              .from('attendance_logs')
              .update({
                actual_out: outTimestamp,
            ...
            supabase
              ...
              .insert({
                crew_id: log.user_id,
                violation_type: 'Terlambat Absen Pulang > 3 Jam',
                notes: 'Sistem otomatis menutup shift karena tidak clock out lebih dari 3 jam.',
                penalty_points: 10,
                outlet_id: log.outlet_id
              }).then(() => {});

            const targetCrewProf = profileMap[log.user_id];
            if (targetCrewProf) {
              const curPts = targetCrewProf.total_points !== null && targetCrewProf.total_points !== undefined ? Number(targetCrewProf.total_points) : 100;
              const nextPts = Math.max(0, curPts - 10);
              supabase
                .from('user_profiles')
                .update({ total_points: nextPts })
                .eq('id', log.user_id)
                .then(() => {});
            }
          ...
          if (p) {
            const isCurrentlyBreaking = Boolean(log.break_start_time) && !Boolean(log.break_end_time);
            ...
            let stationName = isMgr ? 'Manager Duty' : (p.station_placement || 'Station Staff');
            
            if (!isMgr && log.status_in && log.status_in.includes(' - ')) {
              const lastPart = log.status_in.substring(log.status_in.lastIndexOf(' - ') + 3).trim();
            ...
            const finalStationKey = matchedStation || stationName;

            activeMembersArray.push({
              id: p.id,
              name: p.full_name,
              avatar: p.avatar,
              station: finalStationKey,
            ...
            if (isCurrentlyBreaking) {
              ...
              stationCounts[finalStationKey] = (stationCounts[finalStationKey] || 0) + 1;
            }
          }
        }

        const totalIC = staffCount + managerCount;
        const totalStandby = Math.max(0, totalIC - totalBreakActive);

        setActiveShiftMembers(activeMembersArray);
        setActiveShiftStats({
          totalScheduledCrew: totalIC,
          totalActiveNow: totalStandby,
  ...
  const handleOpenStationDetail = (stName) => {
    const matchedCrew = activeShiftMembers.filter(m => {
      if (stName === 'Manager Duty') {
        const r = (m.role || '').toLowerCase();
        return m.station === 'Manager Duty' || r.includes('manager') || r.includes('leader');
      }
      return m.station?.toLowerCase() === stName.toLowerCase() || 
             m.station?.toLowerCase().includes(stName.toLowerCase().replace('station ', ''));
    });

    setStationCrewModal({
      isOpen: true,
      stationName: stName,
      crewList: matchedCrew
    });
  };
      ...
      if (currentLog && currentLog.actual_in && !currentLog.actual_out) {
        ...
        if (currentLog.status_in && currentLog.status_in.includes('Shift ')) {
          try {
            ...
            if (!isNaN(parsedHour)) {
              setSelectedShiftHour(parsedHour);
  ...
  const fetchLiveBreakData = useCallback(async () => {
    ...
    try {
      ...
      let logsQuery = supabase
        .from('attendance_logs')
        .select('id, user_id, break_start_time, break_end_time, actual_in, actual_out, discipline_status, is_outside_radius, distance_meters, status_in, outlet_id')
        .not('actual_in', 'is', null)
        .is('actual_out', null)
        .gte('actual_in', past24Hours);
      ...
      if (profilesData) {
        ...
        if (logsData && logsData.length > 0) {
          const activeBreaks = logsData.filter(l => Boolean(l.break_start_time) && !Boolean(l.break_end_time)).map((log, index) => {
            ...
            if (log.status_in && log.status_in.includes(' - ')) {
              ...
              if (parts[parts.length - 1]) currentStation = parts[parts.length - 1].trim();
            }
        ...
        const activeWBs = profilesData.filter(p => Boolean(p.current_izin_start)).map(wb => {
  ...
  // ================= REKAP KEHADIRAN FLEKSIBEL & RINCIAN DETAIL =================
  const fetchCutoffAttendance = useCallback(async () => {
    setIsFetchingCutoff(true);
    try {
      const startIso = new Date(`${filterStartDate}T00:00:00`).toISOString();
      const endIso = new Date(`${filterEndDate}T23:59:59.999`).toISOString();

      let profQuery = supabase
        .from('user_profiles')
        .select('id, full_name, avatar, station_placement, role, outlet_id, total_points');

      let logsQuery = supabase
        .from('attendance_logs')
        .select('id, user_id, actual_in, actual_out, created_at, outlet_id, status_in, status_out')
        .not('actual_in', 'is', null)
        .gte('actual_in', startIso)
        .lte('actual_in', endIso);
      ...
      const [{ data: profs }, { data: logs }, { data: viols }] = await Promise.all([
        profQuery,
        logsQuery,
        violQuery
      ]);
      ...
      if (profs) {
        const attendanceData = profs.map(p => {
          const pLogs = (logs || []).filter(l => l.user_id === p.id);
          const uniqueWorkDays = new Set(
            pLogs.map(l => (l.actual_in || l.created_at).substring(0, 10))
          ).size;

          const pViols = (viols || []).filter(v => v.crew_id === p.id);
          let mangkirCount = 0;
          let sakitCount = 0;
          let izinCount = 0;
          let cutiCount = 0;
          ...
          return {
            ...
            name: p.full_name,
            avatar: p.avatar,
            role: p.station_placement || p.role || 'Crew',
            totalHadir: uniqueWorkDays,
            mangkir: mangkirCount,
            sakit: sakitCount,
            izin: izinCount,
            cuti: cutiCount,
            points: p.total_points ?? 100,
            records: detailRecords
          };
        });

        setCutoffAttendanceList(attendanceData.sort((a, b) => b.totalHadir - a.totalHadir));
      }
    } catch (e) {
      console.error("Gagal load cut-off attendance:", e);
    } finally {
  ...
  const fetchLeaderboard = useCallback(async () => {
    ...
    try {
      ...
      let logsQuery = supabase
        .from('attendance_logs')
        .select('id, user_id, created_at, break_start_time, break_end_time, actual_in, actual_out, discipline_status, penalty_points, status_in, status_out, is_outside_radius, distance_meters, outlet_id');
      ...
      const processData = (profileList) => {
        ...
        const leaderboardArray = profileList.map(person => {
          ...
          const personLogs = (logs || []).filter(l => {
            ...
            const logDate = l.created_at || l.actual_in;
          ...
          personLogs.forEach(log => {
            const rawDate = log.actual_in || log.created_at;
            const dateKey = rawDate ? rawDate.substring(0, 10) : '';
            if (dateKey && log.actual_in) shiftDates.add(dateKey);
            ...
            const statusInLower = (log.status_in || '').toLowerCase();
            const disciplineLower = (log.discipline_status || '').toLowerCase();
            const statusOutLower = (log.status_out || '').toLowerCase();

            const isLate = statusInLower.includes('terlambat') || disciplineLower.includes('terlambat') || disciplineLower.includes('late');
            if (isLate) {
              if (dateKey) datesWithInfractions.add(dateKey);
              const match = statusInLower.match(/terlambat\s*(\d+)m?/i);
              let mins = match && match[1] ? parseInt(match[1]) : (log.penalty_points ? Number(log.penalty_points) : 1);
              if (mins <= 0 && log.penalty_points) mins = Number(log.penalty_points);
              if (mins > 0) {
                lateCount += 1;
                ...
                userInfractionHistory.push({
                  ...
                  note: log.status_in || 'Terlambat masuk kerja'
                });
              }
            }
            ...
            } else if (disciplineLower === 'overbreak') {
              if (dateKey) datesWithInfractions.add(dateKey);
              overBreakCount += 1;
            ...
            if (log.is_outside_radius || disciplineLower.includes('ghosting')) {
              if (dateKey) datesWithInfractions.add(dateKey);
              ghostingCount += 1;
              totalDeduction += 15;
              userInfractionHistory.push({
            ...
            if (disciplineLower.includes('terlambat absen pulang > 3 jam') || statusOutLower.includes('>3 jam') || statusOutLower.includes('> 3 jam')) {
              if (dateKey) datesWithInfractions.add(dateKey);
              totalDeduction += 10;
              userInfractionHistory.push({
                ...
                date: formattedDate,
                time: `${formattedTime} WITA`,
                note: log.status_out || 'Melanggar batas maksimal waktu toleransi kepulangan'
              });
            } else if (statusOutLower.includes('pulang cepat') || disciplineLower.includes('pulang lebih awal')) {
              if (dateKey) datesWithInfractions.add(dateKey);
              ...
              userInfractionHistory.push({
                ...
                date: formattedDate,
                time: `${formattedTime} WITA`,
                note: log.status_out || 'Pulang lebih awal dari jadwal'
              });
            } else if (disciplineLower.includes('lupa absen pulang') || statusOutLower.includes('lupa')) {
              if (dateKey) datesWithInfractions.add(dateKey);
              totalDeduction += 10;
              userInfractionHistory.push({
                ...
                date: formattedDate,
                time: `${formattedTime} WITA`,
                note: 'Sesi di-close otomatis oleh sistem'
              });
            }
          });
          ...
          shiftDates.forEach(dateStr => {
            if (!datesWithInfractions.has(dateStr)) cleanDaysCount += 1;
          });

          const disciplineBonus = cleanDaysCount * 5;
          let calculatedPoints = (100 + disciplineBonus) - totalDeduction;

          let pts = calculatedPoints;
          if (selectedMonth !== currentMonthYear && historyMap[person.id] !== undefined) {
            pts = Number(historyMap[person.id]);
          }
          ...
          } else if (cleanDaysCount > 0) {
            statusDescription = `✓ ${cleanDaysCount}x Shift Tertib (+${disciplineBonus} Pts)`;
          } else {
  ...
  const handleViolationTypeChange = (newType) => {
    setReportViolationType(newType);
    if (newType === 'Mangkir (Tanpa Keterangan)') {
      setReportPenaltyPoints('25');
    } else if (newType === 'Sakit' || newType === 'Izin' || newType === 'Cuti') {
      setReportPenaltyPoints('0');
    } else if (reportPenaltyPoints === '25' || reportPenaltyPoints === '0') {
  ...
  const handleSubmitOperationalViolation = async (e) => {
    ...
    if (!isNonDisciplinary && !reportEvidenceImage) {
      return alert("⚠️ Wajib mengunggah foto dokumentasi pendukung sebelum mengirim laporan indisipliner.");
    }
    ...
    try {
      ...
      if (deduction > 0) {
        ...
        await supabase
          .from('user_profiles')
          .update({ total_points: Math.max(0, currentPts - deduction) })
          .eq('id', reportTargetCrewId);
      }
  ...
  const handleOpenCutoffDetail = (crewData) => {
    setSelectedCutoffDetail(crewData);
    setShowCutoffDetailModal(true);
  };
  ...
  const fetchAttendanceSummaryList = useCallback(async () => {
    ...
    try {
      let logsQuery = supabase
        .from('attendance_logs')
        .select('id, user_id, created_at, actual_in, actual_out, status_in, status_out, discipline_status, penalty_points, outlet_id')
        .not('actual_in', 'is', null)
        .order('created_at', { ascending: false })
        .limit(80);
      ...
      if (!lError && !pError && logs && profiles) {
        ...
        const formatted = logs.map(l => {
          ...
          let lateMinutes = 0;
          if (l.status_in && l.status_in.toLowerCase().includes('terlambat')) {
            const match = l.status_in.match(/Terlambat (\d+)m/);
            lateMinutes = match && match[1] ? parseInt(match[1]) : (l.penalty_points ? Number(l.penalty_points) : 0);
          }

          let earlyMinutes = 0;
          if (l.status_out && l.status_out.toLowerCase().includes('pulang cepat')) {
            const matchOut = l.status_out.match(/\((\d+)m\)/);
            earlyMinutes = matchOut && matchOut[1] ? parseInt(matchOut[1]) : 0;
          }
  ...
  const fetchAllCrewLogs = useCallback(async (loadMore = false, pageNum = 0) => {
    ...
    try {
      ...
      let logsQuery = supabase
        .from('attendance_logs')
        .select('id, user_id, created_at, break_start_time, break_end_time, image_url, after_break_image_url, actual_in, outlet_id')
  ...
  }, [activeTab, logSubTab, selectedBranchId, selectedMonth, filterStartDate, filterEndDate]);
  ...
  }, [user?.id, selectedBranchId, isAreaManager]);
  ...
  const openCamera = async (mode) => {
    if (mode === 'START_BREAK') {
      if (activeLogId) {
        const { data: currentSessionLog } = await supabase
          .from('attendance_logs')
          .select('break_start_time')
          .eq('id', activeLogId)
          .maybeSingle();

        if (currentSessionLog && currentSessionLog.break_start_time) {
          alert("🔒 Akses Ditolak: Anda sudah mengambil jatah 1 kali break untuk shift ini.");
          return;
        }
      }
    }
    ...
    try {
      ...
      if (cameraMode === 'IN') {
        ...
        if (targetLogId) {
          ...
          checkInHr = refTime.getHours();
          
          if (latestActiveLog?.status_in && latestActiveLog.status_in.includes('Shift ')) {
            try {
              const parsed = parseInt(latestActiveLog.status_in.split('Shift ')[1].split(':')[0]);
              if (!isNaN(parsed)) checkInHr = parsed;
            } catch(e) {}
          }

          const breakDurationSec = (checkInHr === 22) ? 1800 : 3600;
        ...
        const { data: latestActiveLog } = await supabase
          .from('attendance_logs')
          .select('id, break_start_time, actual_in, status_in')
          .eq('user_id', user.id)
          .is('actual_out', null)
          .gte('actual_in', past24Hours)
          .order('created_at', { ascending: false })
        ...
        if (targetLogId && latestActiveLog?.break_start_time) {
          ...
          if (latestActiveLog?.status_in && latestActiveLog.status_in.includes('Shift ')) {
            try {
              const parsed = parseInt(latestActiveLog.status_in.split('Shift ')[1].split(':')[0]);
              if (!isNaN(parsed)) checkInHour = parsed;
            } catch(e) {}
          }
          ...
          if (isOver) {
            ...
            const updatedPoints = Math.max(0, Number(existingPoints) - penaltyPoints);
        ...
        const { data: latestActiveLog } = await supabase
          .from('attendance_logs')
          .select('id, actual_in, status_in, outlet_id')
          .eq('user_id', user.id)
          .is('actual_out', null)
          .gte('actual_in', past24Hours)
          .order('created_at', { ascending: false })
        ...
        if (targetLogId && latestActiveLog?.actual_in) {
          ...
          const targetRequiredHours = (checkInHour === 22) ? 8 : 9;

          let statusOutText = 'Sesuai Jadwal';
          let disciplineStatusOut = 'Shift Selesai';
          let penaltyPoints = 0;
          let financialLoss = 0;

          if (workedHours > (targetRequiredHours + 3)) {
            const lateOutMinutes = Math.floor((workedHours - targetRequiredHours) * 60);
            statusOutText = `Terlambat Absen Pulang > 3 Jam (${Math.floor(lateOutMinutes / 60)}j ${lateOutMinutes % 60}m)`;
            disciplineStatusOut = 'Terlambat Absen Pulang > 3 Jam';
            penaltyPoints = 10;
            
            await supabase.from('operational_violations').insert({
              crew_id: user.id,
              violation_type: 'Terlambat Absen Pulang > 3 Jam',
              notes: `Clock out terlambat lebih dari 3 jam dari jam shift seharusnya (${workedHours.toFixed(1)} jam kerja).`,
              penalty_points: 10,
              outlet_id: latestActiveLog.outlet_id
            });
          } else if (workedHours < targetRequiredHours) {
            const missingMinutes = Math.floor((targetRequiredHours * 60) - (workedHours * 60));
            statusOutText = `Pulang Cepat (${missingMinutes}m)`;
            disciplineStatusOut = 'Pulang Lebih Awal';
            penaltyPoints = Math.min(30, Math.max(5, Math.floor(missingMinutes / 10) * 5));
            financialLoss = missingMinutes * 1000;
          }

          await supabase.from('attendance_logs').update({
            actual_out: timestampIso,
            status_out: statusOutText,
            discipline_status: disciplineStatusOut,
            penalty_points: penaltyPoints,
            financial_loss_amount: financialLoss
          }).eq('id', targetLogId);
          ...
          if (penaltyPoints > 0) {
            ...
            await supabase
              .from('user_profiles')
              .update({ total_points: Math.max(0, Number(existingPts) - penaltyPoints) })
              .eq('id', user.id);
          }
        }
      }
      ...
      if (activeTab === 'all-logs') {
        if (logSubTab === 'summary') await fetchAttendanceSummaryList();
        if (logSubTab === 'cutoff_attendance') await fetchCutoffAttendance();
      }
      ...
      if (cameraMode === 'IN') {
        alert("✓ Presensi MASUK Berhasil Tersimpan!\n\n⚠️ PENTING: Jangan lupa absen di Talenta juga ya!");
      } else if (cameraMode === 'OUT') {
        alert("✓ Presensi PULANG Berhasil Tersimpan! Terima kasih atas dedikasi hari ini.\n\n⚠️ PENTING: Jangan lupa absen di Talenta juga ya!");
      } else {
        alert("✓ Data Presensi & Kedisiplinan Berhasil Diperbarui!");
      }
      ...
      <div className="w-full max-w-md bg-[#F8FAFC] min-h-screen flex flex-col relative pb-20">
        ...
        {activeTab === 'absen' && (
          <div className="flex-1 px-4 py-4 space-y-3.5">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <h1 className="text-sm font-black tracking-tight text-slate-900">{profile?.full_name || 'Memuat Akun...'}</h1>
                  </div>
                </div>
            ...
            {/* BANNER REMINDER TALENTA */}
            <div className="bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200/80 rounded-2xl p-3 shadow-2xs flex items-center gap-2.5">
              <div className="p-2 bg-amber-500 text-white rounded-xl text-xs font-black shadow-xs flex-shrink-0">
                <FiCheckCircle className="text-base" />
              </div>
              <div>
                <p className="text-[10px] font-black text-amber-900 uppercase tracking-wider">Integrasi Talenta</p>
                <p className="text-[11px] text-amber-800 font-bold leading-tight">
                  Jangan lupa absen di <span className="underline decoration-amber-500 font-black">Talenta</span> juga ya setelah melakukan presensi di sini! ✨
                </p>
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-200/80 rounded-2xl p-4 shadow-2xs space-y-3.5">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div 
                  onClick={() => handleOpenStationDetail('Staff Duty')} 
                  className="bg-gradient-to-br from-indigo-50/70 to-slate-50 border border-indigo-100/70 rounded-xl p-2.5 flex items-center gap-2.5 cursor-pointer hover:border-indigo-300 transition-all"
                >
                ...
                <div 
                  onClick={() => handleOpenStationDetail('Manager Duty')} 
                  className="bg-gradient-to-br from-emerald-50/70 to-slate-50 border border-emerald-100/70 rounded-xl p-2.5 flex items-center gap-2.5 cursor-pointer hover:border-emerald-300 transition-all"
                >
              ...
              <div className="space-y-2 pt-1">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[8px] font-bold text-slate-400 bg-slate-50 px-1.5 py-0.5 rounded border border-slate-100">Klik untuk Cek Nama</span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 max-h-64 overflow-y-auto pr-0.5">
                  {Object.entries(activeShiftStats.stationCounts).map(([stationName, count]) => {
                    ...
                    return (
                      <div 
                        key={stationName} 
                        onClick={() => handleOpenStationDetail(stationName)}
                        className={`border rounded-xl p-2.5 flex flex-col justify-between transition-all min-h-[58px] cursor-pointer ${
                          isZero 
                            ? 'bg-slate-50/60 border-slate-200/50 hover:bg-slate-100/80 opacity-75' 
                            : 'bg-white hover:bg-indigo-50/70 hover:border-indigo-300 border-slate-200/80 shadow-2xs'
                        }`}
                      >
                        <span className={`text-[10px] font-extrabold leading-snug break-words ${isZero ? 'text-slate-500' : 'text-slate-800'}`}>
                          {stationName}
                        </span>
                        <div className="flex justify-end mt-1.5">
                          <span className={`text-[9px] font-black font-mono px-2 py-0.5 rounded-md shadow-2xs ${
                            isZero 
              ...
              <div className="space-y-3">
                ...
                {canReportViolation && (
                  <div className="bg-gradient-to-br from-slate-900 to-slate-950 text-white rounded-2xl p-4 shadow-md space-y-3 border border-slate-800">
                    ...
                    >
                      <FiAlertTriangle className="text-xs" /> Buka Form Laporan / Presensi Kru
                    </button>
                  </div>
                )}
              </div>
              ...
              {hasCheckedIn && !hasCheckedOut && (
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-2.5 flex items-start gap-2">
                  ...
                  <p className="text-[10px] text-slate-600 font-semibold leading-relaxed">
                    Target shift: <span className="font-black text-slate-800">{requiredWorkHours} Jam Kerja</span>. Batas toleransi absen pulang maksimal <span className="font-black text-rose-600">3 Jam</span> setelah jadwal selesai. Keterlambatan lebih dari 3 jam dianggap indisipliner (-10 Poin).
                  </p>
                </div>
              )}
            </div>
                ...
                <div className="space-y-3">
                  {timeLeft < 0 ? (
                    ...
                    <div className="space-y-2 text-center">
                      ...
                      <p className="text-[9px] text-slate-400 font-extrabold uppercase tracking-widest">Sisa Alokasi Waktu Istirahat ({requiredWorkHours === 8 ? '30m' : '60m'})</p>
                    </div>
                  )}
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center text-[10px] text-slate-500 font-black uppercase tracking-wider">
                    <span className="flex items-center gap-1.5">
                      ...
                      Alokasi: {requiredWorkHours === 8 || selectedShiftHour === 22 ? '30 Menit (Shift Malam)' : '60 Menit'}
                    </span>
        ...
        {/* ================= MODAL PERSONIL STATION ================= */}
        {stationCrewModal.isOpen && (
          <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
            <div className="bg-white border border-slate-200 rounded-[28px] max-w-sm w-full overflow-hidden shadow-2xl flex flex-col max-h-[85vh]">
              ...
              <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/70">
                <div className="flex items-center space-x-2.5">
                  <div className="p-2 bg-indigo-50 text-indigo-600 rounded-xl text-base">
                    <FiLayers />
                  </div>
                  <div>
                    <h3 className="text-xs font-black text-slate-900 uppercase tracking-tight">Kru On-Duty Station</h3>
                    <p className="text-[10px] text-indigo-600 font-bold">{stationCrewModal.stationName}</p>
                  </div>
                </div>
                <button 
                  onClick={() => setStationCrewModal({ isOpen: false, stationName: '', crewList: [] })}
              ...
              <div className="p-4 flex-1 overflow-y-auto space-y-2.5">
                {stationCrewModal.crewList.length === 0 ? (
                  <div className="py-8 text-center text-xs font-semibold text-slate-400">
                    Belum ada kru yang bertugas di station ini saat ini (0 Kru).
                  </div>
                ) : (
                  stationCrewModal.crewList.map((crew, idx) => (
                    <div key={crew.id || idx} className="p-3 bg-slate-50 hover:bg-indigo-50/40 border border-slate-200/80 rounded-2xl flex items-center justify-between transition-colors shadow-2xs">
                      <div className="flex items-center space-x-3">
                        <UserAvatar src={crew.avatar} name={crew.name} />
                        <div>
                          <span className="text-xs font-bold text-slate-900 block">{crew.name}</span>
                          <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider block">{crew.role || 'Crew'}</span>
                          <span className="text-[9px] font-mono text-slate-500 block mt-0.5">
                            Masuk: {new Date(crew.actualIn).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', timeZone: 'Asia/Makassar' })} WITA
                          </span>
                        </div>
                      </div>
                      ...
                      <div>
                        {crew.isBreaking ? (
                          <span className="px-2 py-0.5 bg-amber-50 border border-amber-200 text-amber-700 text-[9px] font-black rounded-md uppercase">
                            Break
                          </span>
                        ) : (
                          <span className="px-2 py-0.5 bg-emerald-50 border border-emerald-200 text-emerald-700 text-[9px] font-black rounded-md uppercase">
                            Standby
                          </span>
                        )}
                      </div>
        ...
        {/* ================= MODAL DETAIL CUT-OFF & NON-HADIR KRU ================= */}
        {showCutoffDetailModal && selectedCutoffDetail && (
          <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
            <div className="bg-white border border-slate-200 rounded-[28px] max-w-sm w-full overflow-hidden shadow-2xl flex flex-col max-h-[85vh]">
              
              <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/70">
                <div className="flex items-center space-x-2.5">
                  <UserAvatar src={selectedCutoffDetail.avatar} name={selectedCutoffDetail.name} />
                  <div>
                    <h3 className="text-xs font-black text-slate-900 uppercase tracking-tight">{selectedCutoffDetail.name}</h3>
                    <p className="text-[9px] text-indigo-600 font-bold">{selectedCutoffDetail.role}</p>
                  </div>
                </div>
                <button 
                  onClick={() => setShowCutoffDetailModal(false)}
                  className="p-1.5 hover:bg-slate-200 rounded-full text-slate-400 transition-colors cursor-pointer"
                >
                  <FiX className="text-lg"/>
                </button>
              ...
              <div className="p-4 border-b border-slate-100 bg-slate-50/40">
                ...
                <div className="grid grid-cols-4 gap-1.5 text-center">
                  <div className="bg-rose-50 border border-rose-200/80 rounded-xl p-2">
                    ...
                    <p className="font-mono text-sm font-black text-rose-950">{selectedCutoffDetail.mangkir}</p>
                  </div>
                  <div className="bg-amber-50 border border-amber-200/80 rounded-xl p-2">
                    <p className="text-[8px] font-bold text-amber-600 uppercase">Sakit</p>
                    <p className="font-mono text-sm font-black text-amber-950">{selectedCutoffDetail.sakit}</p>
                  </div>
                  <div className="bg-blue-50 border border-blue-200/80 rounded-xl p-2">
                    <p className="text-[8px] font-bold text-blue-600 uppercase">Izin</p>
                    <p className="font-mono text-sm font-black text-blue-950">{selectedCutoffDetail.izin}</p>
                  </div>
                  <div className="bg-emerald-50 border border-emerald-200/80 rounded-xl p-2">
                    <p className="text-[8px] font-bold text-emerald-600 uppercase">Cuti</p>
                    <p className="font-mono text-sm font-black text-emerald-950">{selectedCutoffDetail.cuti}</p>
                  </div>
                </div>
              </div>
              ...
              {/* TIMELINE RINCIAN */}
              <div className="p-4 flex-1 overflow-y-auto space-y-2.5">
                <p className="text-[8px] font-black uppercase text-slate-400 tracking-wider">Daftar Catatan Ketidakhadiran</p>
                {selectedCutoffDetail.records.length === 0 ? (
                  <div className="py-6 text-center text-xs font-semibold text-slate-400">
                    Tidak ada catatan izin/sakit/mangkir pada periode ini. Kehadiran disiplin! 🌟
                  </div>
                ) : (
                  selectedCutoffDetail.records.map((rec, i) => (
                    <div key={i} className="bg-slate-50 border border-slate-200/80 rounded-2xl p-3 space-y-1.5 shadow-2xs">
                      <div className="flex items-center justify-between">
                        <span className={`text-[8px] font-black px-2 py-0.5 rounded-md border uppercase ${rec.badge}`}>
                          {rec.type}
                        </span>
                        <span className="text-[9px] font-mono font-bold text-slate-500">{rec.date}</span>
                      </div>
                      {rec.note && (
                        <p className="text-[10px] text-slate-700 font-medium bg-white p-2 rounded-xl border border-slate-100 leading-relaxed">
                          {rec.note}
                        </p>
                      )}
                      {rec.evidence && (
        ...
        {/* ================= MODAL INPUT PELANGGARAN & ABSENSI KHUSUS ================= */}
        {showReportViolationModal && (
          <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
            <div className="bg-white border border-slate-200 rounded-[28px] max-w-sm w-full overflow-hidden shadow-2xl p-5 space-y-3.5 max-h-[90vh] overflow-y-auto">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="text-xs font-black text-slate-900 uppercase tracking-wider">
                      Input Indisipliner & Absensi
                    </h3>
                    <p className="text-[9px] text-slate-400 font-medium">Catat pelanggaran SOC, mangkir, izin, sakit, atau cuti</p>
                  </div>
                </div>
              ...
              <form onSubmit={handleSubmitOperationalViolation} className="space-y-3">
                ...
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-slate-500 uppercase tracking-wider">Pilih Kru yang Bersangkutan</label>
                  <select
                ...
                {/* JENIS KETERANGAN */}
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-slate-500 uppercase tracking-wider">Jenis Keterangan / Laporan</label>
                  <select
                    value={reportViolationType}
                    onChange={(e) => handleViolationTypeChange(e.target.value)}
                    className="w-full h-10 bg-slate-50 border border-slate-200 rounded-xl px-3 text-xs font-semibold text-slate-900 outline-none cursor-pointer"
                  >
                    <optgroup label="Pelanggaran Operasional">
                    ...
                    <optgroup label="Status Ketidakhadiran">
                      <option value="Mangkir (Tanpa Keterangan)">Mangkir / Alfa (-25 Pts)</option>
                ...
                {/* PENGURANGAN POIN */}
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-slate-500 uppercase tracking-wider">Pengurangan Poin</label>
                  <select
                    value={reportPenaltyPoints}
                    disabled={reportViolationType === 'Mangkir (Tanpa Keterangan)' || ['Sakit', 'Izin', 'Cuti'].includes(reportViolationType)}
                    onChange={(e) => setReportPenaltyPoints(e.target.value)}
                    className={`w-full h-10 border border-slate-200 rounded-xl px-3 text-xs font-semibold outline-none ${
                      reportViolationType === 'Mangkir (Tanpa Keterangan)' || ['Sakit', 'Izin', 'Cuti'].includes(reportViolationType)
                        ? 'bg-slate-100 text-slate-500 cursor-not-allowed'
                        : 'bg-slate-50 text-slate-900'
                    }`}
                  >
                    <option value="0">0 Poin (Tidak Ada Pemotongan)</option>
                    <option value="5">-5 Poin (Ringan)</option>
                    <option value="10">-10 Poin (Sedang)</option>
                    <option value="15">-15 Poin (Berat)</option>
                    <option value="25">-25 Poin (Mangkir / Alfa)</option>
                  </select>
                </div>

                {/* RINCIAN KEJADIAN */}
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-slate-500 uppercase tracking-wider">Rincian / Catatan Keterangan</label>
                  <textarea
                    rows={2}
                    value={reportNotes}
                    onChange={(e) => setReportNotes(e.target.value)}
                    placeholder={
                      reportViolationType === 'Mangkir (Tanpa Keterangan)'
                        ? 'Tidak hadir kerja tanpa konfirmasi/alasan jelas...'
                        : ['Sakit', 'Izin', 'Cuti'].includes(reportViolationType)
                        ? 'Keterangan izin/cuti/surat dokter...'
                        : 'Contoh: Tidak memakai sarung tangan saat handling dimsum...'
                    }
                    required
                ...
                {/* UPLOAD FOTO BUKTI */}
                <div className="space-y-1 pt-0.5">
                  <label className="text-[9px] font-black text-slate-700 uppercase flex items-center gap-1">
                    <FiCamera className="text-orange-600" /> 
                    Foto Bukti Dokumentasi {['Sakit', 'Izin', 'Cuti'].includes(reportViolationType) ? '(Opsional)' : '(Wajib)'}
                  </label>
                  
                  {reportEvidenceImage ? (
                    <div className="relative rounded-2xl overflow-hidden border border-slate-200">
                      <img src={reportEvidenceImage} alt="Bukti" loading="lazy" decoding="async" className="w-full h-28 object-cover" />
                      <button 
                        type="button" 
                    ...
                    <label className="w-full border-2 border-dashed border-slate-300 hover:border-orange-500 bg-slate-50 p-3 rounded-2xl flex flex-col items-center justify-center gap-1 cursor-pointer transition-colors">
                      <FiUploadCloud className="text-lg text-slate-400" />
                ...
                <button
                  ...
                  className="w-full bg-orange-600 hover:bg-orange-700 text-white font-black py-3 rounded-2xl text-xs uppercase tracking-wider shadow-md transition-all active:scale-[0.98] cursor-pointer disabled:opacity-60 mt-1"
                >
                  {isSubmittingReport ? 'Menyimpan Laporan...' : 'Kirim & Simpan Laporan'}
                </button>
              </form>
            </div>
          </div>
          ...
          <div className="flex-1 px-4 py-4 space-y-3.5">
            ...
            <div className="bg-slate-200/80 p-1 rounded-2xl flex gap-1 shadow-inner">
              <button
                ...
                className={`flex-1 py-2 rounded-xl text-xs font-black uppercase tracking-wider transition-all flex items-center justify-center gap-1 cursor-pointer ${logSubTab === 'photo' ? 'bg-white text-indigo-600 shadow-xs' : 'text-slate-500 hover:text-slate-800'}`}
              >
                <FiImage className="text-xs" /> Log Foto
              </button>
              <button
                onClick={() => setLogSubTab('summary')}
                className={`flex-1 py-2 rounded-xl text-xs font-black uppercase tracking-wider transition-all flex items-center justify-center gap-1 cursor-pointer ${logSubTab === 'summary' ? 'bg-white text-indigo-600 shadow-xs' : 'text-slate-500 hover:text-slate-800'}`}
              >
                <FiList className="text-xs" /> Jam Presensi
              </button>
              <button
                onClick={() => setLogSubTab('cutoff_attendance')}
                className={`flex-1 py-2 rounded-xl text-xs font-black uppercase tracking-wider transition-all flex items-center justify-center gap-1 cursor-pointer ${logSubTab === 'cutoff_attendance' ? 'bg-slate-900 text-white shadow-xs' : 'text-slate-500 hover:text-slate-800'}`}
              >
                <FiCalendar className="text-xs" /> Rekap Periode
              </button>
            </div>
            ...
            {logSubTab === 'cutoff_attendance' ? (
              <div className="space-y-3">
                ...
                <div className="bg-white border border-slate-200/80 rounded-2xl p-3.5 shadow-2xs space-y-2.5">
                  <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                    <span className="text-[10px] font-black uppercase tracking-wider text-slate-700 flex items-center gap-1">
                      <FiFilter className="text-indigo-600" /> Filter Rentang Tanggal:
                    </span>
                    <button 
                      onClick={() => {
                        const def = getDefaultCutoffRange();
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="space-y-0.5">
                      <label className="text-[8px] font-extrabold uppercase text-slate-400">Dari Tanggal</label>
                      <input 
                        type="date" 
                        value={filterStartDate} 
                        onChange={(e) => setFilterStartDate(e.target.value)}
                        className="w-full bg-slate-50 border border-slate-200 text-slate-800 font-bold text-xs px-2.5 py-1.5 rounded-xl outline-none"
                      />
                    </div>
                    <div className="space-y-0.5">
                      <label className="text-[8px] font-extrabold uppercase text-slate-400">Sampai Tanggal</label>
                      <input 
                        type="date" 
                        value={filterEndDate} 
                ...
                <div className="flex items-center justify-between px-1">
                  <div>
                    <h3 className="text-xs font-black text-slate-900 uppercase tracking-wider">Total Presensi Kru</h3>
                    <p className="text-[8px] text-slate-400 font-bold">Klik nama kru untuk melihat rincian alasan &amp; foto surat</p>
                  </div>
                  <button 
                    onClick={fetchCutoffAttendance} 
                ...
                {isFetchingCutoff ? (
                  <div className="text-center py-8 text-xs text-slate-400 font-medium animate-pulse">Menghitung total kehadiran...</div>
                ) : cutoffAttendanceList.length === 0 ? (
                  <div className="bg-white border border-slate-200/70 rounded-2xl p-6 text-center text-xs text-slate-400 font-medium shadow-xs">
                    Belum ada data kehadiran pada rentang tanggal yang dipilih.
                  </div>
                ) : (
                  <div className="bg-white border border-slate-200/70 rounded-2xl divide-y divide-slate-100 shadow-xs overflow-hidden">
                    {cutoffAttendanceList.map((item, idx) => (
                      <div 
                        key={item.id} 
                        onClick={() => handleOpenCutoffDetail(item)}
                        className="p-3.5 flex items-center justify-between hover:bg-indigo-50/40 cursor-pointer transition-colors"
                      >
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                          ...
                          <div className="flex items-center gap-1 text-[8px] font-bold pl-4 pt-0.5">
                            ...
                            <span className={`px-1.5 py-0.2 rounded border ${item.sakit > 0 ? 'bg-amber-100 text-amber-800 border-amber-200' : 'bg-slate-100 text-slate-400 border-slate-200'}`}>
                              S: {item.sakit}
                            </span>
                            <span className={`px-1.5 py-0.2 rounded border ${item.izin > 0 ? 'bg-blue-100 text-blue-800 border-blue-200' : 'bg-slate-100 text-slate-400 border-slate-200'}`}>
                              I: {item.izin}
                            </span>
                            <span className={`px-1.5 py-0.2 rounded border ${item.cuti > 0 ? 'bg-emerald-100 text-emerald-800 border-emerald-200' : 'bg-slate-100 text-slate-400 border-slate-200'}`}>
                              C: {item.cuti}
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="text-right flex flex-col items-end gap-1">
                          <span className="bg-emerald-50 text-emerald-700 text-xs font-black px-2.5 py-1 rounded-xl border border-emerald-200">
                            {item.totalHadir} Hari Hadir
                          </span>
                          <span className="text-[8px] font-bold text-slate-400 font-mono">
                            Skor: {item.points} Pts
                          </span>
                        </div>
                      </div>
                    ))}
                    ...
                    {attendanceSummaryList.map((item) => (
                      <div key={item.id} className="p-3.5 flex items-center justify-between hover:bg-slate-50/70 transition-colors">
                        ...
                        <div className="text-right space-y-1">
                          ...
                          <div>
                            {item.statusIn.toLowerCase().includes('terlambat') ? (
                              <span className="text-[8px] font-black uppercase px-2 py-0.5 rounded bg-amber-100 text-amber-800 inline-block border border-amber-200">
                                Terlambat {item.lateMinutes > 0 ? `${item.lateMinutes} Menit` : 'Masuk'}
                              </span>
                            ) : item.statusOut.toLowerCase().includes('pulang cepat') ? (
                              <span className="text-[8px] font-black uppercase px-2 py-0.5 rounded bg-orange-100 text-orange-800 inline-block border border-orange-200">
                                Pulang Cepat {item.earlyMinutes > 0 ? `(${item.earlyMinutes}m)` : ''}
                              </span>
                            ) : (item.statusOut.toLowerCase().includes('> 3 jam') || item.statusOut.toLowerCase().includes('>3 jam')) ? (
                              <span className="text-[8px] font-black uppercase px-2 py-0.5 rounded bg-rose-100 text-rose-800 inline-block border border-rose-200">
                                Terlambat Out &gt; 3 Jam (-10 Pts)
                              </span>
                            ) : (
                              <span className="text-[8px] font-black uppercase px-2 py-0.5 rounded bg-slate-100 text-slate-600 inline-block border border-slate-200">
                                Jadwal Normal
                              </span>
                            )}
                          </div>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <div className="space-y-1">
                            ...
                            {log.image_url ? (
                              ...
                              <div className="w-full aspect-[4/5] rounded-xl bg-slate-50 border border-dashed flex items-center justify-center text-[9px] text-slate-400 font-bold">Foto Telah Diarsip</div>
                            )}
                          </div>
                          ...
                          <div className="space-y-1">
                            ...
                            {log.after_break_image_url ? (
                              ...
                              <div className="w-full aspect-[4/5] rounded-xl bg-slate-50 border border-dashed flex items-center justify-center text-[9px] text-slate-400 font-bold">In Progress / Diarsip</div>
                            )}
                          </div>
                        </div>
                      </div>
        ...
        {showShiftPicker && (
          <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
            <div className="bg-white border border-slate-200 rounded-[28px] max-w-sm w-full overflow-hidden shadow-2xl p-5 space-y-3.5">
              ...
              <div className="bg-amber-50 border border-amber-200/80 rounded-xl p-2.5 flex items-start gap-2 text-amber-900">
                <FiAlertCircle className="text-amber-600 text-sm mt-0.5 shrink-0" />
                <p className="text-[10px] font-bold leading-relaxed">
                  <span className="font-black text-amber-950">PENTING:</span> Pastikan Anda tidak salah memilih jam masuk shift. Sistem akan menghitung keterlambatan &amp; alokasi istirahat berdasarkan pilihan Anda.
                </p>
              </div>
